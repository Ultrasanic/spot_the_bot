...
def get_word_space(embeddings, filename):
    with open(filename, 'r', encoding='utf-8') as f:
        corpus = f.read()
    words = set([w for w in corpus.split() if w in embeddings])
    if not words:
        print(filename)
    space = np.vstack([embeddings[w] for w in words])
    return space
    # return space, words
...
def process(glob_path, embeddings):
    """
        Get features for dataset.

        Parameters:
            dir_path - files of which type to use (glob)
            embeddings
    ...
    features = []
    text_names = []
    hole_embeddings = np.load(f"../holes/RU/words/hole_embeddings.npy", allow_pickle=True).item()

    hole_e_centers = np.vstack([h.mean(axis=0) for h in hole_embeddings.values()])
    for f in tqdm(files):
        word_space = get_word_space(embeddings, f)
...
def main():
    embeddings = np.load('../DATASET/ru_cbow_dictionary.npy', allow_pickle=True).item()
    for bot in [
        'bigbalaboba',
        'biggpt2',
        'bigmGPT',
        ...
        features, _ = process(f'../DATASET/{bot}/*', embeddings)
        np.save(f'features/{bot}.npy', features)
